...
namespace gameplay_renderer {
    ...
    void drawEnemyList(GameLoopContext& ctx) {
        ...
        const float selectedEnemyTop = 330.f;
        ...
        const std::vector<AEnnemy>& ennemies = ctx.stage->getEnnemies();
        const AEnnemy* enemyOnSelectedCell = ctx.hasSelectedGrid ? ctx.stage->getEnemyAt(ctx.selectedGridX, ctx.selectedGridY) : nullptr;
        float currentY = 70.f;
        for (std::size_t i = 0; i < ennemies.size(); ++i) {
        ...
        if (ctx.hasSelectedGrid) {
            ...
            const std::vector<std::vector<int>>& map = ctx.stage->getMap();
            const int mapValue = map[cellY][cellX];
            const sf::Vector2i playerPos = ctx.stage->getPlayer().getPosition();
            ...
            const std::string valueLine = "Map value: " + std::to_string(mapValue);
    ...
    void updateEnemyListHover(GameLoopContext& ctx, int mouseX, int mouseY) {
        ...
        const std::size_t hoveredIndex = static_cast<std::size_t>(relativeY / itemHeight);
        if (hoveredIndex < ennemies.size()) {
            ctx.hoveredEnemyIndex = hoveredIndex;
        } else {
            ctx.hoveredEnemyIndex = std::numeric_limits<std::size_t>::max();
        }
    }
